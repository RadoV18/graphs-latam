...
function generateArraysComplement2(matrix){
...
export const northWestAlgorithm = (matrix, avaliable, demand, optim) =>{
    ...
    while(flag){
        ...
        if(sumValuesMatrix(solution) == valuesInSolution){
            ...
            const valoresSolucion = generateMatrixCostosForComplement(matrix, solution);
            ...
            const matrixComplent = generateSolutionComplement(valoresSolucion, row, column);
            ...
            if(optim){ // Si es verdadero es MAXIMIZAR
                const [op, posicion] = getMaxOfMatrix(matrixDiference);
            }else{
                const [op, posicion] = getMaxOfMatrix(matrixDiference);
            }
            if( op == 0){
            ...
            }else{ //Se tendria que verificar si el minimo es mayor que cero ahi seria un error fatal
                //Se obtiene la posicion del valor minimo del array
                const realValue = getMinForNewSolution(solution,demand,avaliable, posicion[0], posicion[1]);
                newSolution = generateNewSolution(solution, demand, avaliable, posicion[0], posicion[1], realValue);
    ...
    console.log(getSumaProductoMatrix(matrixCosto, solution));
    return solution;
}